...
export class UpdateCustomerController implements UpdateControllerHandler<CustomerRequest> {
    ...
    async handle(request: CustomerRequest, id: number): Promise<HttpResponse> {
        try {
            ...
            const { name, email, phone, avatar, type, status } = request
            ...
            const customer = await this.updateCustomerRepository.update({ name, email, phone, status, avatar, type, company_id: singleCustomer.company_id }, id)
